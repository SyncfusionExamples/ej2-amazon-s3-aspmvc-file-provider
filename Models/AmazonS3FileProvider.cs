...
namespace Syncfusion.EJ2.FileManager.AmazonS3MVCFileProvider
{
    public class AmazonS3FileProvider : AmazonS3MVCFileProviderBase
    {
        ...
        private FileManagerDirectoryContent CreateDirectoryContentInstance(string name, bool value, string type, long size, DateTime createddate, DateTime modifieddate, bool child, string filterpath)
        {
            ...
            {
                string directoryName = new DirectoryInfo(s3Files[i].Key).Name;
                if (directoryName == name)
                {
                    tempFile.DateModified = s3Files[i].LastModified;
                }
            }   
        ...
        {
            return Task.Run(() => AsyncDelete(path, names, data)).Result;
        }
        ...
        public FileManagerResponse TransferItems(string path, string targetPath, string[] names, string[] replacedItemNames, bool isCutRequest, FileManagerDirectoryContent TargetData, params FileManagerDirectoryContent[] data)
        {
            ...
            if (replacedItemNames != null && replacedItemNames.Length == 0 && existFiles.Count > 0)
            {
        ...
        public FileManagerResponse Details(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                {
                    ListingObjectsAsync("/", RootName.Replace("/", "") + path + names[j] + ((data[j].Type == "Folder") ? "/" : ""), false).Wait();
                    foreach (S3Object key in response.S3Objects) 
                    { 
                        sizeValue = sizeValue + key.Size;
                    }
                ...
                if (data.Length == 1 && response.Prefix != null )
                {
                    for (int j = 0; j < s3Files.Count; j++)
                    {
                        if (s3Files[j].Key == response.Prefix)
                        {
                            data[0].DateModified = s3Files[j].LastModified;
                        }
                    }
                }
                ...
                detailFiles = new FileDetails
                {
                    ...
                    Modified = data.Length == 1 ? data[0].DateModified : new DateTime(),
        ...
        public FileManagerResponse Search(string path, string searchString, bool showHiddenItems, bool caseSensitive, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    {
                        if (files[i].FilterPath.Contains("/"))
                        {
                            files[i].FilterPath = files[i].FilterPath.Replace("/", "\\");
                        }
                    }
                }
        ...
        public virtual async Task<FileManagerResponse> AsyncRename(string path, string name, string newName, bool replace, params FileManagerDirectoryContent[] data)
        {
            ...
            {
                await MoveDirectoryAsync((RootName.Replace("/", "") + path + name + "/"), (RootName.Replace("/", "") + path + newName + "/"), data[0].IsFile, true).ConfigureAwait(false);
                try
                {
                    GetBucketList();
                    FileManagerResponse readResponse = new FileManagerResponse();
        ...
        public FileManagerResponse Upload(string path, IList<System.Web.HttpPostedFileBase> uploadFiles, string action, FileManagerDirectoryContent[] data)
        {

            return Task.Run(() => AsyncUpload(path, uploadFiles, action, data)).Result;
        }

        // Uploads the file(s)
        public virtual async Task<FileManagerResponse> AsyncUpload(string path, IList<System.Web.HttpPostedFileBase> uploadFiles, string action, FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                {
                    fsSource.Dispose();
                    uploadFiles[0].SaveAs(Path.Combine(Path.GetTempPath(), fileName));
                    fsSource.Dispose();
                }
        ...
        public virtual FileStreamResult GetImage(string path, string id, bool allowCompress, ImageSize size, params FileManagerDirectoryContent[] data)
        {
            ...
            {
                GetBucketList();
                Task.Run(() => ListingObjectsAsync("/", RootName.Replace("/", "") + path, false));
        ...
        public async Task MoveDirectoryAsync(string sourceKey, string destinationKey, bool isFile, bool deleteS3Objects)
        {
            try
            {
                ...
                do
                {
                    ListObjectsResponse listObjectsResponse = Task.Run(() => client.ListObjectsAsync(listObjectsRequest)).Result;
                    foreach (S3Object s3Object in listObjectsResponse.S3Objects)
                    {
                        ...
                        CopyObjectResponse copyObectResponse = Task.Run(() => client.CopyObjectAsync(copyObjectRequest)).Result;
        ...
        public static async Task ListingObjectsAsync(string delimiter, string prefix, bool childCheck)
        {
            try
            {
                ...
                if (childCheck)
                    childResponse = Task.Run(() => client.ListObjectsAsync(request)).Result;
                else
                    response = Task.Run(() => client.ListObjectsAsync(request)).Result;
            }
            catch (AmazonS3Exception amazonS3Exception) { throw amazonS3Exception; }
        }
